...
# -*- coding: utf-8 -*-
"""
Created on Fri Dec  6 15:03:12 2019
...
###############
###MODIFICARE QUI PER AVERE KERAS, nel caso vogliamo fare il cambio e salvarlo in .pb!!
###############


import tensorflow.keras as tfkeras
def get_submodules_from_kwargs(kwargs):
    backend = kwargs.get('backend', tfkeras.backend)
    layers = kwargs.get('layers', tfkeras.layers)
    models = kwargs.get('models',tfkeras.models)
    utils = kwargs.get('utils', tfkeras.utils)
...
###########aggiunto il dropout alla fine di ogni layer del blocco!!
    
##custom solo per EfficientBettini

##############################
def mb_conv_block(inputs, block_args, activation, drop_rate=0.2, prefix='', ):
...
###per cambiare percentuale dropout intervenire sul dropout name dell'headfer ma soprattutto su

##drop_rate all'interno!!!!!!

def EfficientNet_c(width_coefficient,
                 depth_coefficient,
                 default_resolution,
                 dropout_rate=0.4,
                 drop_connect_rate=0.2,
    ...
    for idx, block_args in enumerate(blocks_args):
        ...
        # The first block needs to take care of stride and filter size increase.
        drop_rate =0.4 #drop_connect_rate #* float(block_num) / num_blocks_total
        
        
        #############intervenire sul drop_rate!!!!
        x = mb_conv_block(x, block_args,
                          activation=activation,
                          drop_rate=drop_rate,
                          prefix='block{}a_'.format(idx + 1))
